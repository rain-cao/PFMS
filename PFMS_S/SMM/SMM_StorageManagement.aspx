<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SMM_StorageManagement.aspx.cs" Inherits="PFMS_S.SteelMeshManagement.SMM_StorageManagement" %>
    ...
    <script type="text/javascript">
        window.onload = function () {
            ...
            if (item == 1) {
                ...
                htmlStr += "&nbsp;<button id=\"btnSearch\" type=\"button\" class=\"layui-btn\" onclick=\"GetStorage()\">查询</button>";
            }
            else if (item == 2) {
                htmlStr = "<button id=\"btnAdd\" type=\"button\" class=\"layui-btn layui-btn-sm\" onclick=\"AddEditStorage(0)\"><i class=\"layui-icon\">+</i></button>";
                htmlStr += "&nbsp;<input id=\"input-condition\" type=\"text\" class=\"layui-input-inline\" style=\"width: 150px; height: 30px;\" lay-verify=\"SearchCondition\" autocomplete=\"off\" placeholder=\"请输入查询条件\" />";
                htmlStr += "&nbsp;<button id=\"btnSearch\" type=\"button\" class=\"layui-btn\" onclick=\"GetStorage()\">查询</button>";
            }
            ...
            layui.use(['table','layer','form'], function () {
                ...
                table.render({
                    ...
                    cols: [[
                        ...
                        { field: 'storage_name', title: '储位名称', align: 'center' },
                        { field: 'address', title: '储位位置', align: 'center' },
        ...
        layui.use('form', function () {
            ...
            form.on('switch(switchStatus)', function () {
                var obj = this;
                $.ajax({
                    url: 'SMM_EditStorageStatus.ashx',
                    data: { 'id': this.value },
                    dataType: 'text',
                    //contentType: 'application/json',
                    type: 'POST',
                    ...
                    success: function (returndata) {
                        if (returndata != 'OK') {
                            layer.alert('储位状态编辑失败!,Message：' + returndata, { icon: 5 });
        ...
        //弹出框新增或编辑储位信息
        function AddEditStorage(id) {
            layui.use(['layer','table'], function () {
                var layer = layui.layer;
                var table = layui.table;
                layer.open({
                    type: 2,
                    title: (id == 0) ? '新增储位' : '储位编辑',
                    ...
                    fixed: true, //固定
                    maxmin: false,
                    content: 'SMM_AddEditStorage.aspx?id=' + id.toString(),
                    end: function () {
                        //GetStorage();
                        table.reload('InfoListReload', {});
                    }
...
<body style="font-family:'Microsoft YaHei';">
    ...
    <div id="div_Table" style="width:99%;margin:0 auto;">
        <table id="SMList" class="layui-hide" style="width:100%;" lay-filter="SMList"></table>
    </div>
</body>
</html>
